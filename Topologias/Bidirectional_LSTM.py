...
from keras.layers import Dense, BatchNormalization, LeakyReLU, LSTM, Bidirectional, Dropout # type: ignore
...
# Camada LSTM com Bach
modelo_bilstm.add(Bidirectional(LSTM(256, return_sequences=True, input_shape=(X_train.shape[1], X_train.shape[2]))))
modelo_bilstm.add(BatchNormalization())
#modelo_bilstm.add(Dropout(0.2))

# Mais camadas LSTM
modelo_bilstm.add(Bidirectional(LSTM(128, return_sequences=True)))
modelo_bilstm.add(BatchNormalization())
#modelo_bilstm.add(Dropout(0.2))

modelo_bilstm.add(Bidirectional(LSTM(64)))
...
modelo_bilstm.save_weights('Bidirectional_LSTM_pesos.weights.h5')

# Previsão e cálculo de métricas
y_pred_Bidirectional_LSTM = modelo_bilstm.predict(X_test)
calculate_metrics(y_test, y_pred_Bidirectional_LSTM)
...
plt.plot(y_pred_Bidirectional_LSTM[:n_points_to_show], label='Predição Bidirectional LSTM', color='orange')
plt.title('Previsão de Precipitação com Bidirectional LSTM')
plt.xlabel('Tempo')
...
plt.savefig('grafico_predicao_Bidirectional_LSTM.png', format='png', dpi=300, bbox_inches='tight')
