...
from keras.layers import Dense, BatchNormalization, LeakyReLU, GRU, Dropout # type: ignore
...
# Camada GRU com Bach
modelo_GRU.add(GRU(256, return_sequences=True, input_shape=(X_train.shape[1], X_train.shape[2])))
modelo_GRU.add(BatchNormalization())
#modelo_GRU.add(Dropout(0.2))

# Mais camadas GRU
modelo_GRU.add(GRU(128, return_sequences=True))
modelo_GRU.add(BatchNormalization())
#modelo_GRU.add(Dropout(0.2))

modelo_GRU.add(GRU(64))
modelo_GRU.add(Dense(50))
modelo_GRU.add(Dense(16, kernel_regularizer=l2(0.01)))
modelo_GRU.add(LeakyReLU(alpha=0.01))
...
modelo_GRU.fit(X_train, y_train, epochs=100, batch_size=64, validation_split=0.2, callbacks=[early_stop])

# Salvar os pesos após o treinamento
modelo_GRU.save_weights('GRU_pesos.weights.h5')

# Previsão e cálculo de métricas
y_pred_GRU = modelo_GRU.predict(X_test)
calculate_metrics(y_test, y_pred_GRU)
...
plt.plot(y_pred_GRU[:n_points_to_show], label='Predição GRU', color='orange')
plt.title('Previsão de Precipitação com GRU')
plt.xlabel('Tempo')
...
plt.savefig('grafico_predicao_GRU.png', format='png', dpi=300, bbox_inches='tight')
